...
class block_cocoon_slider_6 extends block_base {

    /**
     * Start block instance.
     */
    function init() {
        $this->title = get_string('pluginname', 'block_cocoon_slider_6');
    }
    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="main-banner-wrapper">
            <div class="banner-style-one '.$bannerstyle.' home9 owl-theme owl-carousel" '.$ccnConfigDataCarousel.'>';
                  ...
                  for ($i = 1; $i <= $data->slidesnumber; $i++) {
                      ...
                      $files = $fs->get_area_files($this->context->id, 'block_cocoon_slider_6', 'slides', $i, 'sortorder DESC, id ASC', false, 0, 0, 1);
                      if (!empty($data->$sliderimage) && count($files) >= 1) {
                        $mainfile = reset($files);
                        $mainfile = $mainfile->get_filename();
                        $data->$sliderimage = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/{$this->context->id}/block_cocoon_slider_6/slides/" . $i . '/' . $mainfile);
                      } else {
                        $data->$sliderimage = $CFG->wwwroot .'/theme/edumy/images/home/1.jpg';
                          ...
                          <div class="slide slide-one" style="background-image: url(' . $data->$sliderimage . '); ">
					            <div class="container">
                                  ...
                                  if(!empty($data->$btn_type) && $data->$btn_type == '1'){
                                    ...
                                    $text .= '<div class="video_pop df">
            					                        	<div class="video_popup_icon mr25">
            													            <a class="video_popup_btn popup-img popup-youtube" href="'.$data->$video.'">
                                                    <span class="flaticon-play-button-1"></span>
                                                  </a>
            					                          </div>
            						                        <a class="popup-youtube" data-ccn="'.$slide_subtitle.'" href="'.$data->$video.'">' .format_text($data->$slide_subtitle, FORMAT_HTML, array('filter' => true)).'</a>
                                              </div>';
                                  }
                                  $text .= '
                                </div>
                      ...
                      <style type="text/css">
                      .banner-style-one.home9 .owl-dots .owl-dot:nth-child('.$i.'):after {
                        content: "'.format_text($data->$slide_title, FORMAT_HTML, array('filter' => true)). ' ' .format_text($data->$slide_title_2, FORMAT_HTML, array('filter' => true)).'";
                      }
                      </style>
					        </div>
...
$text .= '
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            file_save_draft_area_files($data->$field, $this->context->id, 'block_cocoon_slider_6', 'slides', $i, $filemanageroptions);
        }
    ...
    function instance_delete() {
        global $DB;
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'block_cocoon_slider_6');
        return true;
    }
    ...
    public function instance_copy($fromid) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            if (!$fs->is_area_empty($fromcontext->id, 'block_cocoon_slider_6', 'slides', $i, false)) {
                $draftitemid = 0;
                file_prepare_draft_area($draftitemid, $fromcontext->id, 'block_cocoon_slider_6', 'slides', $i, $filemanageroptions);
                file_save_draft_area_files($draftitemid, $this->context->id, 'block_cocoon_slider_6', 'slides', $i, $filemanageroptions);
            }
        }
